...
Room.prototype.toggleCheckAndCall = function (isCallSituation, callAmount) {
  if (isCallSituation) {
    this.checkBtn.innerHTML = callAmount > 0 ? ('Call ' + callAmount) : 'Call';
  } else {
...
Room.prototype.actionBtnVisibility = function (visible, isInit) {
  if (visible) {
    if (this.foldBtn.style.display === 'none' && !isInit) {
      if (enableSounds) {
        playCardTakeOutFromPackageOne.play();
      }
    }
    this.foldBtn.style.display = '';
    this.checkBtn.style.display = '';
    this.raiseBtn.style.display = '';
  } else {
    if (this.foldBtn.style.display !== 'none' && !isInit) {
      if (enableSounds) {
        playCardTakeOutFromPackageOne.play();
    ...
    // No es tu turno (o ya no lo es): si el panel de monto había quedado
    // abierto de una decisión a medias, se cierra y resetea — así la
    // próxima vez que sea tu turno arranca en 0, no con un monto viejo.
    if (typeof closeRaisePanel === 'function') {
      closeRaisePanel();
    }
  }
};
